...
// API
const char* serverName = "https://iot.dei.estg.ipleiria.pt/ti/ti007/ti/api/api.php";
...
float lastHum = -1000;
int lastFan = -1;
unsigned long manualControlUntil = 0;
const unsigned long MANUAL_CONTROL_DURATION = 10000; // 10 segundos
...
void controlarVentoinha(float temp) {
  if (millis() < manualControlUntil) {
    // Ainda no modo manual, não muda estado automático
    return;
  }
        ...
        if (sep != -1) {
          ...
          if (nome == "ventoinha") {
            ...
            if (digitalRead(FAN_PIN) != novoEstado) {
              digitalWrite(FAN_PIN, novoEstado);
              fanStatus = (novoEstado == HIGH) ? 1 : 0;
              Serial.printf("Ventoinha controlada manualmente: %d\n", fanStatus);
        
              // Ativa o modo manual para bloquear controlo automático
              manualControlUntil = millis() + MANUAL_CONTROL_DURATION;
            }
          }
